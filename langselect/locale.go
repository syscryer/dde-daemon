...
func (ls *LangSelect) getDefaultLocale() (locale string, ok bool) {
	...
	if !dutils.IsFileExist(_DEFAULT_LOCALE_FILE) {
		Logger.Errorf("'%s' not exist", _DEFAULT_LOCALE_FILE)
		return
	}

	contents, err := ioutil.ReadFile(_DEFAULT_LOCALE_FILE)
	if err != nil {
		Logger.Errorf("ReadFile '%s' failed: %v", _DEFAULT_LOCALE_FILE, err)
		return
	}
...
func (ls *LangSelect) getLocaleInfoList() (list []localeInfo) {
	...
	defer keyFile.Free()
	if _, err := keyFile.LoadFromFile(_ALL_LANG_FILE); err != nil {
		return list
	}
